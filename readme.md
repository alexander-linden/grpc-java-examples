...
gRPC uses HTTP/2 as a transport protocol along with Protocol Buffers (Protobuf) as a binary serialization framework and RPC interface description language. These foundations give gRPC performance and capability advantages over RESTful services, which typically use transferring textual JSON messages over the HTTP/1.1 protocol.
...
* `EchoServiceBlockingV2Stub`: to make blocking calls (it supports unary calls as a stable feature and all 3 streaming calls as experimental features), but it can throw a checked `StatusException` instead of a runtime `StatusRuntimeException`.
...
In this example, the request is provided as a method parameter, and responses are returned as an iterator. With this communication pattern, the client blocks while calling the `serverStreamingEcho` method and waits for the call to complete. A call to this stub either returns responses from the server or throws a `StatusRuntimeException`, in which case the gRPC error is encoded as a `Status` instance. After the call is completed, the channel is shut down to ensure that the underlying resources (threads and TCP connections) are released.
